...
void main() {
  group('WireFormat', () {
    test('prekey message round-trips, including the one-time prekey id', () async {
      ...
      final header = RatchetHeader(
        dhPublicKey: await (await CryptoAlgorithms.x25519.newKeyPair()).extractPublicKey(),
        previousChainLength: 3,
        messageNumber: 7,
      );
      final ratchetMessage = RatchetMessage(header: header, ciphertext: [1, 2, 3, 4, 5]);
      ...
      expect(decoded.senderIdentityAgreementKey.bytes, (await identityKeyPair.extractPublicKey()).bytes);
      expect(decoded.senderEphemeralKey.bytes, (await ephemeralKeyPair.extractPublicKey()).bytes);
    ...
    test('normal message round-trips', () async {
      ...
      final ratchetMessage = RatchetMessage(header: header, ciphertext: [10, 20, 30]);
